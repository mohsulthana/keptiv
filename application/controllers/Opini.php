...
class Opini extends CI_Controller {
	...
	public function index($page = '')
	{
		...
		if ($data != false) {
			...
			if ($page != '') {
				...
				$result['sub_page'] = 'opini';
			}
			$this->load->view('opini', $result);
		}
	}
	...
	public function getOpini()
	{
		...
		if ($sub_page != 'opini') {
			$categories = $this->mhome->whereCategories('opini');
			$sub_category = $categories[0]['sub_category'.$sub_page];
			$opini = $this->mhome->getOpiniSubCategory($page, $sub_category);
		}else {
			// $opini = $this->mhome->getOpini($page);
			$opini = $this->mhome->getOpiniSubCategory($page, 'Opini');
		}
		echo json_encode($opini);
	}
