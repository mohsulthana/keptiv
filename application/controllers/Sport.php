...
class Sport extends CI_Controller {
	...
	public function index($page = '')
	{
		...
		if ($data != false) {
			...
			if ($page != '') {
				...
				$result['sub_page'] = 'sport';
			}
			$this->load->view('sport', $result);
		}
	}
	...
	public function getSport()
	{
		...
		if ($sub_page != 'sport') {
			$categories = $this->mhome->whereCategories('sport');
			$sub_category = $categories[0]['sub_category'.$sub_page];
			$sport = $this->mhome->getSportSubCategory($page, $sub_category);
		}else {
			// $sport = $this->mhome->getSport($page);
			$sport = $this->mhome->getSportSubCategory($page, 'Sport');
		}
