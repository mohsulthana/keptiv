...
class Unik extends CI_Controller {
	...
	public function index($page = '')
	{
		...
		if ($data != false) {
			...
			if ($page != '') {
				...
				$result['sub_page'] = 'unik';
			}
			$this->load->view('unik', $result);
		}
	}
	...
	public function getUnik()
	{
		...
		if ($sub_page != 'unik') {
			$categories = $this->mhome->whereCategories('unik');
			$sub_category = $categories[0]['sub_category'.$sub_page];
			$unik = $this->mhome->getUnikSubCategory($page, $sub_category);
		}else {
			// $unik = $this->mhome->getUnik($page);
			$unik = $this->mhome->getUnikSubCategory($page, 'Unik');
		}
		echo json_encode($unik);
	}
